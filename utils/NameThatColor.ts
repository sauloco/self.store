...
export default (color: string) => {
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        const ndf1 = Math.pow(r - +names[i][2], 2) + Math.pow(g - +names[i][3], 2) + Math.pow(b - +names[i][4], 2);
        const ndf2 = Math.pow(h - +names[i][5], 2) + Math.pow(s - +names[i][6], 2) + Math.pow(l - +names[i][7], 2);
...
const init = () => {
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        newNames.push([color, name, rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]]);
    }

    return newNames
}

export const names = init();